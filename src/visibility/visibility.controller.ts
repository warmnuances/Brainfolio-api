...
export class VisibilityController {
    ...
    @Get()
    getExistingList(@GetUser() user:Userv2):Promise<Visibility[]>{
        const username = user.username;
        return this.visibilityService.getUserList(username);
    }
    
    @Post()
    createToken(@GetUser() user: Userv2, @Body(ValidationPipe) body:createTokenDto):Promise<Visibility>{
        const username = user.username;
